...
class PhotoPage(BasePage):
    ...
    def filter_by_free_license(self):
        menu_button = WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(loc.LICENSE_MENU_BUTTON))
        menu_button.click()
        free_option = WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(loc.FREE_LICENSE_OPTION))
        free_option.click()
    ...
    def select_random_image(self):
        ...
        image = WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(RANDOM_IMAGE))
    ...
    def download_selected_image(self, image, random_index):
        ...
        RANDOM_IMAGE_DOWNLOAD_BUTTON = (By.XPATH, f'(//a[@itemprop="contentUrl"])[{random_index}]'
    ...
    def wait_for_download(self, files_before):
        WebDriverWait(self.driver, 30).until(
            lambda d: set(os.listdir(self.download_path)) != files_before)
    ...
    def add_image_to_collection(self, image, random_index):
        ...
        actions = ActionChains(self.driver)
        actions.move_to_element(image).perform()
        click_create_collection = WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(CLICK_CREATE))
    ...
    def create_new_collection(self):
        ...
        WebDriverWait(self.driver, 5).until(
            ec.element_to_be_clickable(loc.NEW_COLLECTION)).click()
        return unique_name
    ...
    def click_load_more_and_verify_count(self):
        ...
        WebDriverWait(self.driver, 10).until(
            lambda driver: self.get_image_count() > count_before)
    ...
    def verify_collection_link_redirect(self):
        collection_link = WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(loc.COLLECTIONS_LINK))
        collection_link.click()
        WebDriverWait(self.driver, 10).until(
            ec.url_contains("collections"))
