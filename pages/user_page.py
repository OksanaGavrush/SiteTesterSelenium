...
class UserPage(BasePage):
    ...
    def select_random_user_profile(self):
        WebDriverWait(self.driver, 10).until(
            ec.presence_of_all_elements_located(loc.VIEW_PROFILE_LINKS))
    ...
    def follow_user(self):
        follow_button = WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(loc.FOLLOW_BUTTON))
        follow_button.click()
        click_follow = WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(loc.MENU_ITEM))
        click_follow.click()
    ...
    def click_message_button(self):
        message_button = WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(loc.MESSAGE_BUTTON))
    ...
    def enter_message_in_textarea(self, message):
        textarea_element = WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(loc.TEXTAREA))
