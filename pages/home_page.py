...
class HomePage(BasePage):
    ...
    def open(self):
        self.driver.get(loc.HOMEPAGE_URL)
    ...
    def clicking_random_image(self):
        images = self.find_oll(loc.IMAGES)
    ...
    def scroll_right_button(self):
        for _ in range(5):
            ...
            WebDriverWait(self.driver, 10).until(
                ec.element_to_be_clickable(loc.SCROLL_LEFT_BUTTON))
    ...
    def scroll_left_button(self):
        scroll_left_button = WebDriverWait(self.driver, 10).until(ec.visibility_of_element_located
                                                                  (loc.SCROLL_LEFT_BUTTON))
    ...
    def navigate_to_account_page(self):
        WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(loc.PERSONAL_MENU_BUTTON)).click()
        WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(loc.VIEW_PROFILE_LINK)).click()

    @allure.step("Click on 'Account' link")
    def click_account_link(self):
        WebDriverWait(self.driver, 10).until(
            ec.element_to_be_clickable(loc.ACCOUNT_LINK)).click()
    ...
    def click_highlights_button(self):
        activity_button = WebDriverWait(self.driver, 10).until(ec.presence_of_element_located(loc.ACTIVITY_BUTTON))
